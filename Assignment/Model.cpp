...
void Model::loadModel(const HeapString &fileName) {
  ...
  for (SizeType i = 0; i < vertexData.size(); i += 3)
    vertices.pushBack(math::VC3(vertexData[i], vertexData[i + 1],
                                vertexData[i + 2]));

  std::vector<int> indices = plyModel->m_face_data;
  SizeType polysRejected = 0;
  for (SizeType i = 0; i < indices.size(); i += 3) {
    polygons.pushBack(Triangle(&(vertices[(SizeType)indices[i]]),
                               &(vertices[(SizeType)indices[i + 1]]),
                               &(vertices[(SizeType)indices[i + 2]])));
  ...
  FB_LOG_INFO(
      FB_MSG(vertices.getSize(), " vertices, ", polygons.getSize(),
        " polys created, ", polysRejected, " polys rejected"));
  ...
  FB_LOG_INFO(
      FB_MSG("\nModel ", fileName, "\n\t", polygons.getSize(), " polys, ",
        vertices.getSize(), " vertices\n\tMin bounds: ", minBoundsStr,

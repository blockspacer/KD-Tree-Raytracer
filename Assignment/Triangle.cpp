	...
	: vertices{ a, b, c }
{
	...
	if (a->isInf() || a->isNaN() || b->isInf() || b->isNaN() || c->isInf() || c->isNaN())
	{
	...
	fb_assert(uDotV > -lang::NumericLimits<float>::getMax() && uDotV < lang::NumericLimits<float>::getMax() && "Invalid uDotV in a triangle");
...
int Triangle::intersects(Ray &ray) const 
{
	...
	fb_assert(wDotV > -lang::NumericLimits<float>::getMax() && wDotV < lang::NumericLimits<float>::getMax());
	fb_assert(wDotU > -lang::NumericLimits<float>::getMax() && wDotU < lang::NumericLimits<float>::getMax());
	float sI = (this->uDotV * wDotV - this->vDotV * wDotU) / this->denom;
	float tI = (this->uDotV * wDotU - this->uDotU * wDotV) / this->denom;
	fb_assert(sI > -lang::NumericLimits<float>::getMax() && sI < lang::NumericLimits<float>::getMax());
	fb_assert(tI > -lang::NumericLimits<float>::getMax() && tI < lang::NumericLimits<float>::getMax());
	...
	fb_assert(sIPlusTI > -lang::NumericLimits<float>::getMax() && sIPlusTI < lang::NumericLimits<float>::getMax());
